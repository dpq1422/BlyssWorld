...
function abcd(row_no)
{
	...
	var usr=$("#usr"+row_no).val();
	var amt=$("#amt"+row_no).val();
	var bnk=$("#bnk"+row_no).val();
	var pm=$("#pm"+row_no).val();
	...
	if(click3==1)
	{
		$.ajax({
			type: "POST",
			url: "AjaxWalletRequestUpdateServlet",
			data: {'req': req , 'status': status , 'remarks': remarks , 'usr': usr , 'amt': amt , 'bnk': bnk , 'pm': pm },
			dataType: "json",
			...
			success: function( data, textStatus, jqXHR) {
				...
				//window.location.href='WalletRequestsReceivedServlet';
			}	 
		});
	}
}
            ...
            <div class="w3-row-padding w3-margin-top">
            	<div class="w3-col m12 wow bounceIn">
                	<div class="table-box wh w3-left">
						...
						?>
                    	<div class="box-head">
                        	<h3>WALLET REQUESTS RECEIVED FROM USER <span class="w3-right w3-blue w3-center badges"><?php echo $total_records;?></span></h3>
                        </div>
						<div class="table-search-filter wh w3-left">
							<form class="wh w3-left" method="get">
								<ul>
                                    <li>
										<label>User Id</label>
                                        <input name="s1" id="s1" onclick="clrval()" value="<?php echo $s1;?>" type="text" placeholder="User Id" class="w3-input w3-border w3-round">
                                    </li>
						<?php
						if($total_records>0 && $s1!="")
						{
						?>
                                    <li>
										<label>User Name</label>
                                        <input value="<?php echo show_user_name($s1);?>" disabled readonly class="w3-input w3-border w3-round">
                                    </li>
                                    <li>
										<label>User Type</label>
                                        <input value="<?php echo show_user_type_name(show_user_type($s1));?>" disabled readonly class="w3-input w3-border w3-round">
                                    </li>
                                    <li>
										<label>&nbsp;</label>
										<a href="DownloadUserRequestServlet?1=1<?php echo $qr;?>" class="w3-button w3-blue w3-round">Download Excel</a>
                                    </li>  
						<?php
						}
						?>
                        	...
                        	<ul>
								...
								while($user_row=mysql_fetch_array($user_result))
								{
									...
									if($request_status==1)
									{
										...
										{
										$st_link="<input type='hidden' id='req$i' value='".$user_row['request_id']."' /><input type='hidden' id='usr$i' value='".$user_row['user_id']."' /><input type='hidden' id='amt$i' value='".$user_row['deposit_amount']."' /><input type='hidden' id='bnk$i' value='".$user_row['bank_id']."' /><input type='hidden' id='pm$i' value='".$user_row['payment_mode']."' />
										...
										<button onclick=\"abcd($i)\" class=\"w3-button w3-blue w3-round\">Update Request</button>";
										}
										else
										{
                                ...
                                <li>
                                	...
                                	<span><?php echo $user_row['request_date'];?></span>
										...
										<table class="wh">
											<tr>
												<td width="35%"valign="top">
													<p><strong>REF NO:-</strong> <?php echo $user_row['ref_no'];?></p>
													<p><strong>AMOUNT:-</strong> <?php echo $user_row['deposit_amount'];?></p>
													<p><strong>REQUEST DATE & TIME:-</strong> <?php echo $user_row['request_date'];?> at <?php echo $user_row['request_time'];?></p>
													<p><strong>USER REMARKS:-</strong> <?php echo $user_row['client_remarks'];?></p>
													<p><strong>USER TIME:-</strong> <?php echo $user_row['client_updates'];?></p>
												</td>
												<td width="35%" valign="top">
													<p>&nbsp;</p>
