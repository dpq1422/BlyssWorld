...
if($m_id!="" && $txnst!="" && $m_id>10003000)
{
	...
	include_once('zf-TxnSource6RcApi.php');
	$resulted_data=check_order_status62($m_id);
	
	if($arr[0]!=0 && $arr[1]!=0 && $arr[2]!=0 && $arr[3]==6)
	{
		$order=$arr[2];
		...
		if($resulted_data=='{"data":[{"orderId":0,"status":0,"mobile":0,"amount":0,"error_code":"112","TransId":0,"service":0,"reqTime":0,"bal":0,"resText":"Invalid order id"}]}')
		{
			$rc_stat=4;// failed
		}
