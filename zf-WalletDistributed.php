...
function update_user_balance_check($user)
{
	...
	$query="select * from $bankapi_child_wallet.distribution where user_id='$user' order by wallet_date,wallet_time; ";
	$result=mysql_query($query);
...
function show_distributed_balance()
{
	require('zc-gyan-info-admin.php');
	require('zc-commons-admin.php');
	$query="select * from $bankapi_child_wallet.distribution where user_id=$main_admin order by wallet_date desc,wallet_time desc, wallet_id desc limit 0,1 ";
	$result=mysql_query($query);
	$bal=0;
	while($row=mysql_fetch_array($result))
	{
...
function show_rt()
{
	require('zc-gyan-info-admin.php');
	require('zc-commons-admin.php');
	$query="select * from $bankapi_child_wallet.realtime order by wallet_date desc,wallet_time desc, wallet_id desc limit 0,1 ";
	$result=mysql_query($query);
	$bal=0;
	while($row=mysql_fetch_array($result))
	{
