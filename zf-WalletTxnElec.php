...
function txn_recharge($userid, $service, $mobile, $operator_code, $operator, $circle, $amount, $source)
{
	...
	if(($balance_retailer+$show_user_limit+$unpaid)<$ded_ret)
	...
	if($err=="")
	{
		...
		{
			$type=6;
			$desc="ELECTRICITY, $mobile, $operator, $amount";
		}
		else if($service==117)
		{
			$type=7;
			$desc="WATER, $mobile, $operator, $amount";
		}
		else if($service==114)
		{
			$type=8;
			$desc="GAS, $mobile, $operator, $amount";
		}
		else if($service==113)
		{
			$type=9;
			$desc="LANDLINE/BROADBAND, $mobile, $operator, $amount";
		}
		else if($service==116)
		{
			$type=10;
			$desc="DATACARD, $mobile, $operator, $amount";
		}
		else if($service==115)
		{
			$type=11;
			$desc="INSURANCE, $mobile, $operator, $amount";
		}
		...
		else
		{
			...
			$qry4="";
			$qry4="update $bankapi_child_txn.txn_rc set mid='$m_id' where order_id='$order_id';";
			mysql_query($qry4);
			...
			if($source==6)
			{
				//deduct_amount_for_admin
				$qry7="";
				$qry7="INSERT INTO $bankapi_parent_wallet.rt_rech_blyss(wallet_date, wallet_time, client_id, user_id, client_order_id, source_order_id, transaction_type, transaction_description, amount_pre, amount_cr, amount_dr, amount_bal) value('$datetime_date', '$datetime_time', '$clientdbid', '$userid', '$order_id', '$m_id', '2', 'RC Order No. $order_id, $desc generated by user id $userid of client id $clientdbid', '$balance_admin', '0', '$ded_admin', '$remain_admin')";
				mysql_query($qry7);
			}
			...
			*/
			...
			$qry12="insert into $bankapi_parent_txn.txn_rc_margin(rc_id, created_on, client_id, source_id, type, service_id, amount, admin_rate, client_rate, admin_comm) value ('$m_id', '$datetime_datetime', '$clientdbid', '$source', '$type', '$service', '$amount', '$deduction_admin', '$deduction_client', $deduction_client-$deduction_admin)";
			mysql_query($qry12);
		}
	}
	return $err;
}
function txn_recharge_with_op1to5_fetch_bill($userid, $mobile, $operator_code, $op1, $op2, $op3, $op4, $op5)
{
	require('zc-gyan-info-admin.php');
	require('zc-commons-admin.php');
	
	$qr1="insert into $bankapi_parent_txn.fetch_bill values(NULL,'$clientdbid','$userid','bill no : $mobile <br>operator code : $operator_code','$datetime_datetime','0');";
	mysql_query($qr1);
	$generated_id=mysql_insert_id();
	
	include_once('zf-TxnSource4RcApi.php');
	$response=start_recharge_with_op1to5("FB".$generated_id,$mobile,"10",$operator_code, "", $op1, $op2, $op3, $op4, $op5);
	
	//$response='{"data":{"orderId":85124201,"status":"FAILED","mobile":"Z21UF470775K","amount":"11.00","operatorId":null,"error_code":"171","service":"PSPCL - PUNJAB","bal":"15836.3735","creditUsed":0,"resText":"Unable to fetch bill details","billAmount":""}}';
	
	//$response='{"data":{"orderId":85124215,"status":"FAILED","mobile":"Z21UF470775K","amount":"11.00","operatorId":0,"error_code":"128","service":"PSPCL - PUNJAB","bal":"15836.3735","creditUsed":"13.0000","resText":"urid is duplicate","billAmount":null}}';
	
	$results= json_decode($response, true);
	$result=$results['data'];
	$resp_status=$result['status'];
	$resp_bill_amount=$result['billAmount'];
	$resp_bill_name=$result['billName'];
	$qr2="update $bankapi_parent_txn.fetch_bill set response ='$response' where fbid ='$generated_id';";
	mysql_query($qr2);
	return $resp_bill_amount."@#@".$resp_bill_name;
}

function txn_recharge_with_op1to5($userid, $service, $mobile, $operator_code, $operator, $circle, $amount, $source, $op1, $op2, $op3, $op4, $op5)
{
	$err="";
	$txn_status=1;
	...
	//$field="id_12";
	$deduction_retailer=show_user_rc_rate_surcharge($service,$operator_id,$userid);//$field
	$deduction_client=show_client_rc_rate_surcharge($source,$service,$operator_id);
	$deduction_admin=show_admin_rc_rate_surcharge($source,$service,$operator_id);
	...
	if($err=="")
	{
		...
		else
		{
			...
			////////////////////do with $limit
			$qry2="";			
			$qry2="INSERT INTO $bankapi_child_txn.txn_rc(created_on, user_id, mobile_number, operator, circle, source, type, pre_bal, amount, deducted_amt, post_bal, updated_on, rc_status, mid, tid, ip, opv1, opv2, opv3, opv4, opv5) value ('$datetime', '$userid', '$mobile', '$operator', '$circle', '$source', '$type', '$balance_retailer', '$amount', '$ded_ret', '$remain_ret', '$datetime', '$txn_status', '$m_id', '$t_id', '$user_ip', '$op1', '$op2', '$op3', '$op4', '$op5')";
			$order_id=generate_rc_client_txn($qry2);
			if($order_id%1000==0)
			{
				//include_once('../functions/_zsms.php');
			...
			$qry3="";			
			$qry3="INSERT INTO $bankapi_parent_txn.txn_rc(client_id, order_id, created_on, user_id, mobile_number, operator, circle, source, type, amount, pre_bal, deducted_amt, post_bal, updated_on, mrc_status, tid, ip, opv1, opv2, opv3, opv4, opv5) value('$clientdbid', '$order_id', '$datetime', '$userid', '$mobile', '$operator', '$circle', '$source', '$type', '$amount', '$balance_admin', '$ded_admin', '$remain_admin', '$datetime', '$txn_status', '$t_id', '$user_ip', '$op1', '$op2', '$op3', '$op4', '$op5')";
			$m_id=generate_rc_admin_txn($qry3);
			...
			{	
				include_once('zf-TxnSource4RcApi.php');
				$response=start_recharge_with_op1to5($m_id,$mobile,$amount,$operator_code,$circle, $op1, $op2, $op3, $op4, $op5);
				$results= json_decode($response, true);
				$result=$results['data'];
...
function refund_amount($oid,$mid,$tid,$refund_source_tid,$source)
{
	...
	{
		// get order deducted amount of admin wallet with wallet id
		$query_ref="select * from $bankapi_parent_wallet.rt_rech_blyss where user_id='$uid' and client_id='$clientdbid' and client_order_id='$oid' and transaction_type='2' order by wallet_id desc limit 0,1";
		$result_ref=mysql_query($query_ref);
		while($rs_ref = mysql_fetch_assoc($result_ref))
		{
			$admin_charges=$rs_ref['amount_dr'];
