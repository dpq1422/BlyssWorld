<?php
	...
	if($s1!="")
	{
		$file="wallet-request-of-user-$s1-".date("Y/m/d")."-".date("h:i:sa").".csv";
		
		$i=0;
		$csv_header = '"Request Id","Request Date","Deposit Date","Bank Name","Payment Method","Ref.No.","Amount","Status","User Remarks","System Remarks"';
		$csv_header .= "\n";
		$csv_row ='';
		...
		while($user_row=mysql_fetch_array($user_result))
		{
			...
			$csv_row .= '"'.$user_row['request_id'].'","'.$user_row['request_date'].'","'.$user_row['deposite_date'].'","'.show_bank_name($user_row['bank_id']).'","'.$payment_mode.'","'.$user_row['ref_no'].'","'.$user_row['deposit_amount'].'","'.$request_status.'","'.$user_row['client_remarks'].'","'.$user_row['admin_remarks'].'"';
			$csv_row .= "\n";
		}
