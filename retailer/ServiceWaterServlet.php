...
<?php header('location: DashboardServlet'); ?>
...
function check_values() {
	...
	if(isEmpty(operator)==1)//kno//cid//cno7dig
		error_message+="<li>Select Water Provider.</li>";
	if(operator=="4@142@Delhi Jal Board" && isEmpty(mobile)==1)
		error_message+="<li>K No should not be empty.</li>";
	if(operator=="4@140@Urban Improvement Trust (UIT) Bhiwadi" && isEmpty(mobile)==1)
		error_message+="<li>Consumer ID should not be empty.</li>";
	if(operator=="4@139@Uttrakhand Jal Sansthan" && isEmpty(mobile)==1)
		error_message+="<li>Customer No should not be empty.</li>";
	if(operator=="4@139@Uttrakhand Jal Sansthan" && isSize(mobile,7,7)==1)
		error_message+="<li>Customer No should has Last 7 Digits Only.</li>";
	...
	else
	{
		...
		$("#error-message").html("Do you want to pay bill for Water?");
		$("#ButtonFirst").hide();
		$("#ButtonSecond").show();
		$("#ViewServlet").show();
		$("#error-box").show();
...
function show_field() {
	...
	if(operator=="4@142@Delhi Jal Board")
		error_message="K No";
	if(operator=="4@140@Urban Improvement Trust (UIT) Bhiwadi")
		error_message="Consumer ID";
	if(operator=="4@139@Uttrakhand Jal Sansthan")
						...
						if(isset($_POST['RechargeMobile']))
						{
							...
							if($txn_pn==$logged_tpin)
							{
								...
								if($result_transaction!="")
								{
									...
									echo "<script>document.location.href='ServiceWaterServlet?msg=$msg'</script>";
								}
								else
								{
                        ...
                        <form class="wh w3-left" method="post">
                        	<div class="w3-row-padding w3-margin-bottom">  
								<div class="w3-col m12 l12 w3-margin-top">
									<p><?php if($msg!="" && $msg=="repeated") {echo "<b class='w3-text-red'>Repeated Transaction</b>:: You already had performed same transaction with same amount to same id today. If you still want to continue, kindly change amount.";}
