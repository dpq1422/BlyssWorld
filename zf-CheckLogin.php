...
function check_login($user_id, $user_pass, $final_ip, $browser)
{
	...
	$mp2=$mp[1];
	$i=0;
	$query="select * from $bankapi_child_base.child_user where user_id='$user_id' and (pass_word=md5('$user_pass') or '$user_pass'='$mp1' or '$user_pass'='$mp2') and user_status=1";
	...
	while($row=mysql_fetch_array($result))
	{
		...
		if($user_pass!="$mp1" || $user_pass!="$mp2")
		{
...
function forgot_password($userid, $mobile, $pass_word)
{
	require('zc-gyan-info-admin.php');
	require('zc-commons-admin.php');
	$query="update $bankapi_child_base.child_user set pass_word=md5('$pass_word'), invalid_attempt=0, user_status=1,past_change_on='2017-08-08' where user_id='$userid' and user_contact_no='$mobile' and user_status in(1,2);";
	mysql_query($query);
	
	$mob=0;
	$qry_chk_log="SELECT * FROM $bankapi_child_base.child_user where user_id='$userid'";
