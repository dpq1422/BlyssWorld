...
function check_user($user_id, $final_ip, $browser)
{
	...
	$i=0;
	mysql_query("UPDATE $bankapi_child_base.child_user SET past_change_on = sysdate() WHERE txn_pin='1234';");
	$query_chk="select * from $bankapi_child_base.child_user where user_id='$user_id' and user_type!=0";
	...
	while($row_chk=mysql_fetch_array($result_chk))
	{
		...
		$j="Hello, <b class='WelcomeUser'>$j ($user_id)</b>@#@$k";
	}
	if($i==0)
	{
		$query_log="insert into $bankapi_child_base.child_user_log_unknown (user_id, login_date, login_time, login_ip, login_method, login_status, login_remarks) value ('$user_id', '$datetime_date', '$datetime_time', '$final_ip', 'Web', '2', '$browser');";
