...
function abcd(row_no)
{
	...
	if(click3==1)
	{
		$.ajax({
			type: "POST",
			url: "../AjaxWalletDistributorRequestUpdateServlet",
			data: {'req': req , 'status': status , 'remarks': remarks , 'usr': usr , 'amt': amt , 'bnk': bnk },
			dataType: "json",
            ...
            <div class="w3-row-padding w3-margin-top">
            	<div class="w3-col m12 wow bounceIn">
                	<div class="table-box wh w3-left">
						...
						/**************************/
						$s1=$s2=$s3=$s4="";
						$cond=" where 1=1 and bank_id='$logged_user_id' ";
						...
						$total_records=show_requests_count_dist($cond);
						$user_result=show_requests_data_dist($cond, $start_from, $num_rec_per_page);
						$qr="&s1=$s1&s2=$s2&s3=$s3&s4=$s4&search=search";
						$i=0;ini_set('display_errors', 1);
						?>
                    	<div class="box-head">
                        	<h3>WALLET REQUESTS RECEIVED FROM TEAM <span class="w3-right w3-blue w3-center badges"><?php echo $total_records;?></span></h3>
                        </div>
                        	...
                        	<ul>
								...
								while($user_row=mysql_fetch_array($user_result))
								{
									...
									if($request_status==1)
									{
										...
										if($wbal>=$user_row['deposit_amount'])
										{
										$st_link="<input type='hidden' id='req$i' value='".$user_row['request_id']."' /><input type='hidden' id='usr$i' value='".$user_row['user_id']."' /><input type='hidden' id='amt$i' value='".$user_row['deposit_amount']."' /><input type='hidden' id='bnk$i' value='".$user_row['bank_id']."' />
									...
									$llvvll=show_user_type($user_row['user_id']);
									$llvvllnm=show_level_name($llvvll);
								?>
                                <li>
                                	...
                                	<div class="address<?php echo $i;?> inner-add wh w3-left">
										<table class="wh">
											<tr>
												...
												<td width="35%" valign="top">
													<p>&nbsp;</p>
													<p><strong>YOUR REMARKS:-</strong> <?php echo $user_row['admin_remarks'];?></p>
													<p><strong>YOUR TIME:-</strong> <?php echo $user_row['admin_updates'];?></p>
												</td>
