...
function recheck6(order,txnst,row)
{
	$("#error-message").html($("#error-message").html()+"<br>for <b>order no. : "+order+"</b>");
	$("#error-box").show();
	var loc=document.location.href;
            ...
            <div class="w3-row-padding w3-margin-top">
            	<div class="w3-col m12 wow bounceIn">
                	<div class="table-box wh w3-left">
						...
						/**************************/
						$s1=$s2=$s3=$s4="";
						$cond=" where 1=1 and type in(3,4,5,6,7,8,9,10,11) ";
                        ...
                        <div class="table-div wh w3-left">
                        	<ul>
								...
								while($mrc_row=mysql_fetch_array($mrc_result))
								{
									...
									if($mmt_type==3)
										$mmt_type="<b class='w3-text-blue'>MOBILE RC</b>";
									else if($mmt_type==4)
										$mmt_type="<b class='w3-text-blue'>DTH RC</b>";
									else if($mmt_type==5)
										$mmt_type="<b class='w3-text-green'>POSTPAID</b>";
									else if($mmt_type==6)
										$mmt_type="<b class='w3-text-orange'>ELECTRICITY</b>";
									else if($mmt_type==7)
										$mmt_type="<b class='w3-text-orange'>WATER</b>";
									else if($mmt_type==8)
										$mmt_type="<b class='w3-text-orange'>GAS</b>";
									else if($mmt_type==9)
										$mmt_type="<b class='w3-text-green'>LANDLINE / BROADBAND</b>";
									else if($mmt_type==10)
										$mmt_type="<b class='w3-text-green'>DATACARD</b>";
									else if($mmt_type==11)
										$mmt_type="<b class='w3-text-orange'>INSURANCE</b>";
									
									$mmt_status2=$mmt_status=$mrc_row['mrc_status'];
									if($mmt_status==-2)
									...
									$recheck="";
									$dt1=strtotime($mrc_row['created_on']);
									$dt2=time();
									$oidcheck=$mrc_row['mrc_id'];
									if($mrc_row['source']==4 && $mmt_status2!=5 && $dt2-$dt1<=259200)
									{
										$oidcheck=$mrc_row['mrc_id'];
										$recheck=" &nbsp;<img title='Re-Check Status of Order $oidcheck' src='img/refresh-icon.png' onclick='recheck(\"$oidcheck\",\"$mmt_status2\",\"$i\")'>";
									}
									else if($mrc_row['source']==6 && $mmt_status2!=5 && $dt2-$dt1<=259200)
									{
										$oidcheck=$mrc_row['mrc_id'];
										$recheck=" &nbsp;<img title='Re-Check Status of Order $oidcheck' src='img/refresh-icon.png' onclick='recheck6(\"$oidcheck\",\"$mmt_status2\",\"$i\")'>";
									}
								?>
                                <li>
                                	<span><?php echo $mrc_row['mrc_id'];?></span>
                                    <span><?php echo $mrc_row['created_on'];?></span>
                                    ...
                                    <span><?php echo $mmt_status.$recheck;?></span>
