...
function show_wallet_count($cond)
{
	...
	$query="select * from $bankapi_child_wallet.distribution $cond and date(wallet_date)>date(DATE_ADD(sysdate(), INTERVAL -3868200 SECOND)) and date(wallet_date)>='$wdates'";
...
function show_wallet_data($cond, $start_from=0, $num_rec_per_page=0)
{
	...
	$query="select * from $bankapi_child_wallet.distribution $cond and date(wallet_date)>date(DATE_ADD(sysdate(), INTERVAL -3868200 SECOND)) and date(wallet_date)>='$wdates' order by wallet_id desc ";
	else
	$query="select * from $bankapi_child_wallet.distribution $cond and date(wallet_date)>date(DATE_ADD(sysdate(), INTERVAL -3868200 SECOND)) and date(wallet_date)>='$wdates' order by wallet_id desc LIMIT $start_from, $num_rec_per_page";
	$result=mysql_query($query);
	return $result;
}
