...
if($m_id!="" && $txnst!="" && $m_id>58292)
{
	...
	$order=$arr[0];
	$clientid=$arr[1];
	$clienttype=$arr[2];//bankapi_child1_1001_txn
	$resulted_data=fund_transfer_order_status25($m_id);//$StatusCode,$Status,$Description,$ASTransCode,$ReferenceNumber,$response
	...
	require_once('zc-commons-admin.php');
	if($txn_status!=$txnst)
	{
