<?php
	...
	if(isset($_SESSION['logged_user_id']))
	{
		...
		if($dt1!="" && $dt2!="" && $usid!="" && strlen($usid)==6)
		{
			...
			$i=0;
			$csv_header = '"USER ID","JOINING DATE","DESIGNATION","NAME","CITY","EMAIL","MOBILE","USER STATUS","KYC STATUS","PARENT ID","PARENT NAME","PARENT DESIGNATION","CURRENT WALLET BALANCE","TOTAL TXN","TOTAL AMOUNT","MT TXN","MT AMOUNT","AV TXN","AV AMOUNT","PREPAID TXN","PREPAID AMOUNT","DTH TXN","DTH AMOUNT","POSTPAID TXN","POSTPAID AMOUNT","UTILITY TXN","UTILITY AMOUNT"';
			$csv_header .= "\n";
			$csv_row ='';
			$aa=$bb=$cc=0;
			...
			while($user_row=mysql_fetch_array($user_result))
			{
				...
				$csv_row .= '"'.$user_row['user_id'].'","'.$join_date2.'","'.$user_type.'","'.$user_row['user_name'].'","'.$user_row['city_name'].'","'.$user_row['e_mail'].'","'.$user_row['user_contact_no'].'","'.$user_status.'","'.$kyc.'","'.$pid.'","'.$pnm.'","'.$pdes.'","'.$wb.'","'.$t1.'","'.$t2.'","'.$nomt.'","'.$nomtamt.'","'.$noav.'","'.$noavamt.'","'.$norc.'","'.$norcamt.'","'.$nodth.'","'.$nodthamt.'","'.$nopp.'","'.$noppamt.'","'.$noutl.'","'.$noutlamt.'"';
				$csv_row .= "\n";
			}
			//$csv_row .= '"========","========","========","========","========","========","========","========","========","========","========","========","========","========","========","========","========","========","========","========","========","========","========","========","========","========"';
			//$csv_row .= "\n";
