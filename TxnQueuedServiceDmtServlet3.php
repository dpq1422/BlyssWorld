...
var click=0;
function process5(order,txnst,row)
{
...
function fail5(order,txnst,row)
{
	$("#error-title").html("Processing Order");
	$("#error-message").html("<img src='img/refresh.gif' height='50' align='right' />Please wait few seconds...<br>while make transaction fail for <b>order no. : "+order+"</b>");
	$("#error-box").show();
	var loc=document.location.href;
	loc=loc.split("&selectedrow=")[0];
	click++;
            ...
            <div class="w3-row-padding w3-margin-top">
            	<div class="w3-col m12 wow bounceIn">
                	<div class="table-box wh w3-left">
						...
						/**************************/
						$s1=$s2=$s3=$s4="";
						$cond=" where 1=1 and mmt_status in(-1,-2) and type=1 and source=5 and client_id in('1000','2000') ";
						...
						?>
                    	<div class="box-head">
                        	<h3>QUEUED TRANSACTIONS - Acharya4Blyss - Domestic Money Transfer <span class="w3-right w3-blue w3-center badges"><?php echo $total_records;?></span></h3>
                        </div>
                        <div class="table-div wh w3-left">
                        	<ul>
								...
								while($mmt_row=mysql_fetch_array($mmt_result))
								{
									...
									if($mmt_row['source']==5 && $mmt_status2!=5 && $dt2-$dt1<=2592000 && $mmt_row['bank_ref_no']!=4321)
									{
										$oidcheck=$mmt_row['mmt_id'];
										$recheck=" &nbsp;<img title='Re-Check Status of Order $oidcheck' src='img/refresh-icon.png' onclick='recheck5(\"$oidcheck\",\"$mmt_status2\",\"$oidcheck\")'>";
									}
									...
									{
										if($mmt_row['source']==5 && $_REQUEST['check']=="check" && $expno==$oidcheck && $mmt_status2!=5 && $dt2-$dt1<=2592000 && $mmt_row['type']==1)
										{
											$oidcheck=$mmt_row['mmt_id'];
											$fail=" &nbsp;<button onclick='fail5(\"$oidcheck\",\"$mmt_status2\",\"$oidcheck\")'class='w3-button w3-red w3-round w3-left'>Fail</button>";
											$process=" &nbsp;<button onclick='process5(\"$oidcheck\",\"$mmt_status2\",\"$oidcheck\")'class='w3-button w3-green w3-round w3-right'>Process</button>";
										}
									}
                                ...
                                <li>
                                	<div class="address<?php echo $oidcheck;?> inner-add wh w3-left">
										...
										<table width='100%'>
											...
											<tr><td colspan='8'><?php echo $fail.$process;?><!----></td></tr>
										</table>
                                    </div>
                                </li>
								<?php
